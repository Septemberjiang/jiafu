<template>
  <div class="app-container">
    ...
    >
      <el-table-column label="名称" prop="role_name" align="center" :class-name="getSortClass('id')">
        <template slot-scope="{row}">
          <span>{{ row.role_name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="编码" prop="code" align="center" :class-name="getSortClass('id')">
        <template slot-scope="{row}">
          <span>{{ row.code }}</span>
        </template>
      </el-table-column>
      <el-table-column label="所在部门" prop="dep" align="center" :class-name="getSortClass('id')">
        <template slot-scope="{row}">
          <span>{{ row.dep }}</span>
        </template>
      </el-table-column>
      <el-table-column label="上级角色" prop="superior_role" align="center" :class-name="getSortClass('id')">
        <template slot-scope="{row}">
          <span>{{ row.superior_role }}</span>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="auto" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="上级角色" prop="superior_role">
          <el-input v-model="temp.superior_role" />
        </el-form-item>
      </el-form>
...
<script>
import { fetchList, fetchPv, createArticle, updateArticle } from '@/api/system/role/role'
